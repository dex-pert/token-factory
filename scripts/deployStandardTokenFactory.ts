...
async function main() {
  ...
  const standardToken = await hre.ethers.deployContract("StandardToken");
  await standardToken.waitForDeployment();
  const standardTokenAddress = standardToken.target
  console.log("standardTokenAddress:",standardTokenAddress)
  ...
  const liquidityBuySellFeeTokenFactory = await hre.ethers.deployContract("StandardTokenFactory", [
